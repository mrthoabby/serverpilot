...
func (s *Server) Start() error {
	...
	mux.Handle("/api/containers/release", s.requireSecureReauth(http.HandlerFunc(s.handleContainerRelease)))
	mux.Handle("/api/containers/delete", s.requireSecureReauth(http.HandlerFunc(s.handleContainerDelete)))
	...
	mux.Handle("/api/compose/projects", s.authMiddleware(http.HandlerFunc(s.handleComposeProjectsList)))
	...
	mux.Handle("/api/system/disk-root-scan", s.authMiddleware(http.HandlerFunc(s.handleDiskRootScan)))
	...
	mux.Handle("/api/terminal/ssh/ws", s.requireReauth(http.HandlerFunc(s.handleTerminalSSHWS)))

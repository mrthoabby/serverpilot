...
(function() {
  ...
  function githubDialog() {
    ...
    openDialog("Secure integration", "Configure GitHub App", '<div class="am-form-grid"><div class="am-field"><label for="amGhLogin">Account login</label><input id="amGhLogin" autocomplete="off" value="' + h(connection.account_login || "") + '"></div><div class="am-field"><label for="amGhType">Account type</label><select id="amGhType"><option>Organization</option><option>User</option></select></div><div class="am-field"><label for="amGhAppID">App ID</label><input id="amGhAppID" type="number" min="1" value="' + h(connection.app_id || "") + '"></div><div class="am-field"><label for="amGhInstallationID">Installation ID</label><input id="amGhInstallationID" type="number" min="1" value="' + h(connection.installation_id || "") + '"></div><div class="am-field am-field-full"><label for="amGhPrivateKey">Private key</label><textarea id="amGhPrivateKey" autocomplete="off" placeholder="-----BEGIN RSA PRIVATE KEY-----"></textarea><small>Encrypted before storage.</small></div><div class="am-field"><label for="amGhWebhook">Webhook secret</label><input id="amGhWebhook" type="password" autocomplete="new-password"></div><div class="am-field"><label for="amGhPAT">GHCR PAT classic</label><input id="amGhPAT" type="password" autocomplete="new-password"><small>Optional for public images.</small></div></div>', '<button class="am-button" value="cancel">Cancel</button><button class="am-button am-button-primary" type="button" id="amSaveGitHub">Save connection</button>');
    document.getElementById("amSaveGitHub").addEventListener("click", async function() {
      var target = this; target.disabled = true;
      try {
        await apiFetch("/api/app-manager/github/configure", { method: "POST", body: { account_login: document.getElementById("amGhLogin").value.trim(), account_type: document.getElementById("amGhType").value, app_id: Number(document.getElementById("amGhAppID").value), installation_id: Number(document.getElementById("amGhInstallationID").value), private_key: document.getElementById("amGhPrivateKey").value, webhook_secret: document.getElementById("amGhWebhook").value, registry_pat: document.getElementById("amGhPAT").value } });
        closeDialog(); showToast("GitHub connection saved", "success"); await reload();
